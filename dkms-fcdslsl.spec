%define module fcdslsl
%define version 3.11.07
%define card "AVM Audiovisuelles|Fritz!Card DSL SL"

Summary: dkms package for %{module} driver
Name: dkms-%{module}
Version: %{version}
Release: %mkrel 6
Source0: ftp://ftp.avm.de/cardware/fritzcrd.dsl_sl/linux/suse.93/fcdslsl-suse93-3.11-07.tar.bz2
Source1: dkms-fcdslsl-use-autoconf-header.patch
Source2: dkms-fcdslsl-dont-redefine-uintptr_t.patch
Source3: dkms-fcdslsl-use-pci_register_driver.patch
Source4: dkms-fcdslsl-remove-DECLARE_MUTEX_LOCKED.patch
Source5: dkms-fcdslsl-update-irq-flags.patch
Source6: dkms-fcdslsl-update-device_interrupt-definition.patch
...
CLEAN="make clean"
AUTOINSTALL="yes"
PATCH[0]="dkms-fcdslsl-use-autoconf-header.patch"
PATCH_MATCH[0]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[1]="dkms-fcdslsl-dont-redefine-uintptr_t.patch"
PATCH_MATCH[1]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[2]="dkms-fcdslsl-use-pci_register_driver.patch"
PATCH_MATCH[2]="^2\.6\.(2[2-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[3]="dkms-fcdslsl-remove-DECLARE_MUTEX_LOCKED.patch"
PATCH_MATCH[3]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[4]="dkms-fcdslsl-update-irq-flags.patch"
PATCH_MATCH[4]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[5]="dkms-fcdslsl-update-device_interrupt-definition.patch"
PATCH_MATCH[5]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
EOF

tar c . | tar x -C $RPM_BUILD_ROOT/usr/src/%module-%version-%release/

for p in %{_sourcedir}/dkms-fcdslsl-use-autoconf-header.patch \
         %{_sourcedir}/dkms-fcdslsl-dont-redefine-uintptr_t.patch \
         %{_sourcedir}/dkms-fcdslsl-use-pci_register_driver.patch \
         %{_sourcedir}/dkms-fcdslsl-remove-DECLARE_MUTEX_LOCKED.patch \
         %{_sourcedir}/dkms-fcdslsl-update-irq-flags.patch \
         %{_sourcedir}/dkms-fcdslsl-update-device_interrupt-definition.patch;
do
